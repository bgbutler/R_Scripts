...
sql = read_file('C:/Users/bbutler/MyStuff/TransactionAnalysis/AllCustomersPrimary.sql')
primaryCust = AP2_QUERY(sql = sql)
head(primaryCust)
write.csv(primaryCust, 'C:/Users/bbutler/MyStuff/TransactionAnalysis/PrimaryCustomers.csv')
...
# make for the actual customers
primaryCust$Generation = apply_gen(primaryCust$Cust_Age)
primaryCust$Generation = order_gen(primaryCust$Generation)
...
plot_bar_generation = function(df, x, y, fill, n, title, theme='theme_white', xmax){
...
plot_bar_generation(df=surveyGen, x=surveyGen$Percent, y=surveyGen$Generation, n=survey_gen,
                    xmax = 75, theme='theme_big', title = 'Survey Participation by Generation')


# for customer base
customerGen = primaryCust %>% dplyr::select(Generation) %>% 
  group_by(Generation) %>% 
  summarise(Count = n()) %>% 
  filter(!is.na(Generation))

# get the total for percentage
customer_gen = sum(customerGen$Count)

# get percentage
customerGen$Percent = customerGen$Count/customer_gen * 100


# plot for current survey
plot_bar_generation(df=customerGen, x=customerGen$Percent, y=customerGen$Generation, n=customer_gen,
                    xmax=45,theme='theme_big', title = 'All Customers by Generation')
...
plot_bar_generation(df=listGen, x=listGen$Percent, y=listGen$Generation, n=list_gen,
                    xmax=45,theme='theme_big', title = 'Email List by Generation')
...
plot_bar_generation(df=originalGen, x=originalGen$Percent, y=originalGen$Generation, n=original_gen,
                    xmax=50, theme='theme_big', title = 'Original List by Generation')
...
plot_bar_generation(df=oldGen, x=oldGen$Percent, y=oldGen$Generation, n=old_gen,
                    xmax=75,theme='theme_big', title = 'Original Survey by Generation')
...
# filter out na
used = data %>% filter(!is.na(UsedMobile))

# used mobile
plot_box(df = used, x=used$Age, y=used$UsedMobile, fill=used$UsedMobile, title = 'Distribution of Mobile Use by Age')
...
# know mobile
knowMobile = data %>% dplyr::select(KnowMobile, Generation) %>% 
  group_by(KnowMobile, Generation) %>% 
  summarise(Count = n())
...
# barchart of counts
rollup_bar = function(df, x, y, fill, title, theme='theme_white', size=1.5, lab=''){
  g = ggplot(df, aes(y = y, x=x, fill = fill)) +
    geom_bar(position = 'dodge', stat = 'identity') + 
    
    geom_text(position = position_dodge(width= 1),
              size = 5, color = 'blue',
              aes(x=Count+3, label=Count, hjust=0)) +
    
    
    scale_fill_manual(values = easternPal) +
    get(theme)() + 
    theme(plot.title = element_text(size = rel(size), face = "bold.italic", color = "blue")) +
    ggtitle(title) +
...
rollup_bar_facet = function(data=data, x, y, fill, title, facet, xmax, theme='theme_white', size=1.5, lab=''){
  df <- data.frame(x = x, y = y, fill = fill, fwrap = facet)
...
# for percentages
bar_percent = function(df, x, y, xmax=40, title, n, fill='darkblue', theme='theme_big_white', size=1.5, lab=''){
...
bar_percent_no_order = function(df, x, y, xmax=40, title, n, fill='darkblue', theme='theme_big_white', size=1.5, labs=''){
...
rollup_bar(df=knowMobile, x=knowMobile$Count, y=knowMobile$KnowMobile, fill=knowMobile$Generation,
           size=3, theme='theme_bryan', title="Customers Who Know About Mobile App")

# check a facet wrap
rollup_bar_facet(data=knowMobile, x=knowMobile$Count, y=knowMobile$KnowMobile, fill=knowMobile$Generation,
                 size=3, theme='theme_bryan',xmax=1000, facet=knowMobile$KnowMobile, title="")


# used mobile
usedMobile = rollup_fun(data, UsedMobile, Generation) %>% filter(!is.na(UsedMobile))

usedMobile$UsedMobile = ordered(usedMobile$UsedMobile, levels = c('No', 'Yes'))
yesMobile = usedMobile %>% filter(UsedMobile == 'Yes')
rollup_bar(df=yesMobile, x=yesMobile$Count, y=yesMobile$UsedMobile, fill=yesMobile$Generation,
                 size=0, theme='theme_bryan',
                 title = "")



noMobile = usedMobile %>% filter(UsedMobile == 'No')
rollup_bar(df=noMobile, x=noMobile$Count, y=noMobile$UsedMobile, fill=noMobile$Generation,
           size=0, theme='theme_bryan', title = "")


################################################################################
# RESET THE DATA TO EXCLUDE THE BOOMERS+ GENERATIONS
...
bar_percent_no_order(df=futureUse, x=futureUse$Percent, y=futureUse$UseAppInFuture, n=use_app_total,
            fill=futureUse$UseAppInFuture ,title='Likely to Use in Future')
...
bar_percent_no_order(df=useApp, x=useApp$Percent, y=useApp$GetSmartPhoneUseApp,
            ...
            title='Use App if Get Smartphone', xmax=65)
...
g = ggplot(awareUse, aes(y = NowMobileAwareLikelyUse, x=Percent, fill = NowMobileAwareLikelyUse)) +
  ...
  geom_text(position = position_dodge(width= 1),
            size = 5, color = 'blue',
            aes(x=Percent+1, label=paste0(format(round(Percent,1)),"%"), hjust=0)) +
...
# make a palette
pal = c('darkblue', 'sienna1', 'royalblue1')
...
# make the dendogram plot
create_cluster(clusDf, k = 3, "Theme Similarities by Generation", max = 50)
...
theme_bryan <- function () { 
  theme(axis.text.x = element_text(size = 10, color = 'blue', angle = 0),
        legend.position = 'bottom',
        axis.text.y = element_text(size = 12, color = 'blue', angle = 0),
        axis.title.x = element_blank(),
        axis.title.y = element_blank(),
        strip.text.x = element_text(color = 'darkblue', size = 10, face = 'bold'),
        strip.background = element_rect(fill = 'light blue'))
...
termDoc = function(text){
  TermDocumentMatrix(text, control = list(tokenize = NLP_tokenizer))
}
...
keyTerm <- "check deposit"
wordAssoc <- findAssocs(DTM, keyTerm, .15)
...
# add topic name
reasons_top_terms$topic_name = ifelse(reasons_top_terms$topic == 1,
                                      "Problems/Like Branch",
...
TDM = termDoc(text = text)
tokenMatrix = as.matrix(TDM)
...
wordcloud(token.d$word, token.d$freq,scale=c(8,.5), min.freq = 1, max.words = 1000,
