...
emailList = data_list %>% select(Customer_Age)

# get all the customers
allCust = read.csv('C:/Users/bbutler/MyStuff/TransactionAnalysis/PrimaryCustomers.csv')
allCust = allCust %>% select(Cust_Age)
...
# get generation for the original list
emailList$Generation = apply_gen((emailList$Customer_Age))
emailList$Generation = order_gen(emailList$Generation)

# do it for all customers
allCust$Generation = apply_gen((allCust$Cust_Age))
allCust$Generation = order_gen(allCust$Generation)
...
# select a palette
easternPal = c('cornflowerblue', 'sienna1', 'darkblue','lightsteelblue1', 'skyblue', 'darkgreen', 'orange')



# basic theme
theme_bryan <- function () { 
  theme(axis.text.x = element_text(size = 12, color = 'blue', angle = 0, face ='bold'),
...
# survey generations
listGen = emailList %>% dplyr::select(Generation) %>% 
  group_by(Generation) %>% 
  summarise(Count = n()) %>% 
  filter(!is.na(Generation))

# get the total for percentage
email_gen = sum(listGen$Count)

# get percentage
listGen$Percent = listGen$Count/email_gen * 100


# plot for current survey
plot_bar_generation(df=listGen, x=listGen$Percent, y=listGen$Generation, n=email_gen,
                    title = 'Email List by Generation')


#### ALL CUSTOMERS
# for customer base
customerGen = allCust %>% dplyr::select(Generation) %>% 
  group_by(Generation) %>% 
  summarise(Count = n()) %>% 
  filter(!is.na(Generation))
...
plot_bar_generation(df=customerGen, x=customerGen$Percent, y=customerGen$Generation, n=customer_gen,
                    title = 'All Customers by Generation')
...
# barchart of counts
rollup_bar = function(df, x, y, fill, title, lab=""){
...
# for percentages stacked color='white'
...
bar_percent_stacked_reorder = function(df, x, y, xmax=100, title, n, fill='darkblue', 
                                       theme='theme_big_white', lab="", order='Count')
  {
...
# just plot the yes
knowMobile = knowMobile %>% filter(KnowMobile == 'Yes')
...
yes = usedMobile %>% filter(UsedMobile == 'Yes')

rollup_bar(df=yes, x=yes$Count, y=yes$UsedMobile, fill=yes$Generation,
           title="")


no = usedMobile %>% filter(UsedMobile == 'No')

rollup_bar(df=no, x=no$Count, y=no$UsedMobile, fill=no$Generation,
           title="")
...
# filter the yes for simpler plots
usedMobile = usedMobile %>% filter(UsedMobile == 'Yes')
...
bar_percent_stacked_reorder(df=whyNot, x=whyNot$Count, y=whyNot$ReasonNotUse, n=n_total, order=whyNot$Count,
                            title='Why Not Use App This Time',fill = whyNot$Generation, xmax = 425)
...
bar_percent_stacked_reorder(df=whyNot, x=whyNot$Count, y=whyNot$ReasonNotUse, n=n_total, title='Why Not Use App This Time',
                            fill = whyNot$Generation, xmax = 425, order=whyNot$Count)
...
# nowAwareUse = rollup_fun(data, NowMobileAwareLikelyUse) %>% 
...
# regular bar
# regular bar without generation
bar_percent_no_order(df=nowAwareUse, x=nowAwareUse$Percent, y=nowAwareUse$NowMobileAwareLikelyUse, xmax=40, n=use_now_total,
                     fill=nowAwareUse$NowMobileAwareLikelyUse, title='Now Aware - Likelihood to Use App')
...
bar_percent_stacked_reorder(df=reasonsNever, x=reasonsNever$Count, y=reasonsNever$MobileReluctance,
                            ...
                            title='Reasons Not Likely to Use Mobile', xmax=500, order=reasonsNever$Percent)
...
n_smartphone = sum(smartPhone$Count)
smartPhone$Percent = smartPhone$Count/n_smartphone * 100
...
bar_percent_stacked(df=smartPhone, x=smartPhone$Count, y=smartPhone$SmartPhone,
                    n=n_smartphone, fill = smartPhone$Generation,
                    title='Smartphone Counts', xmax=900)
