...
write.csv(balWide, file="C:/Users/n846490/Documents/DataScience/balWideDataQ2.csv", row.names=FALSE)
...
#reduce the dataset
balData <- select(balWide, Branch.Number, PerfLevel, PeerGroup, BizTotal, CDIRA, ConsCk,
                  ConsCC, ConsMM, ConsSav, Heloc, HomeEq, Invest, PersLoans)


#merge the data to one
balanceSheet <- branchData <- inner_join(branchData, balData, by = "Branch.Number")
...
balanceSheet$ConsCkLvl <- cut(balanceSheet$BizTotal,
                           c(0,5000000,10000000,15000000,20000000, 100000000), include.lowest = T,
                           labels = c("<$5M", "$5-10M", "$10-15M", "$15-20M", "$20M+"))


balanceSheet$ConsCCLvl <- cut(balanceSheet$ConsCC,
                              c(0,200000,400000,600000,800000, 10000000), include.lowest = T,
                              labels = c("<$200K", "$200-400K", "$400-800K", "$800k-1M", "$1M+"))


balanceSheet$HelocLvl <- cut(balanceSheet$Heloc,
                             c(0,5000000,10000000,15000000,20000000, 100000000), include.lowest = T,
                             labels = c("<$5M", "$5-10M", "$10-15M", "$15-20M", "$20M+"))
...
balanceSheet$PersLoansLvl <- cut(balanceSheet$PersLoans,
                              c(0,200000,400000,600000,800000, 10000000), include.lowest = T,
                              labels = c("<$200K", "$200-400K", "$400-800K", "$800k-1M", "$1M+"))
...
balanceSheet$PeerGroup <- as.numeric(as.character(balanceSheet$PeerGroup))


#set the color scale
binColors <- colorFactor(rainbow(5), balanceSheet$PerfLevel)
binColorsL <- colorFactor(palette = "RdYlBu", balanceSheet$CsatLvl)
binColorsCk <- colorFactor(palette = "Blues", balanceSheet$ConsCkLvl)
binColorsCC <- colorFactor(palette = "Greens", balanceSheet$ConsCCLvl)
binColorsHeloc <- colorFactor(palette = rainbow(6), balanceSheet$HelocLvl)
binColorsBiz <- colorFactor(palette = "RdYlBu", balanceSheet$bizLvl)
binColorsInvest <- colorFactor(palette = "BuGn", balanceSheet$InvestLvl)
binColorsPeer <- colorFactor(palette = "Accent", balanceSheet$PeerGroup)
...
balanceSheet$allData <- paste("<strong>",balanceSheet$Branch.Name, "</strong><br>",
                              ...
                              "Peer Group = ", balanceSheet$PeerGroup, "<br>",
                              "Biz Tot = $" , formatC(balanceSheet$BizTotal,4) , "M", "<br>",
                              "Cons Ck = $", formatC(balanceSheet$ConsCk,4), "M","<br>",
                              "Heloc = $", formatC(balanceSheet$ckRev,4), "M", "<br>", 
                              "Investments = $", formatC(balanceSheet$Invest,4), "M", "<br>",
                              "Pers Loans = $", formatC(balanceSheet$PerLoans,4), "M", sep = "")
...
m <- leaflet() %>%
    ...
    #cons checking  
    addCircleMarkers(data = balanceSheet, lat = ~Lat, lng = ~Lon, 
                     color = ~binColorsCk(ConsCkLvl), popup = balanceSheet$allData,
                     radius = ~ConsCk/2, group = "Cons Ck") %>%
    
    #cons  CC    
    addCircleMarkers(data = balanceSheet, lat = ~Lat, lng = ~Lon, 
                     color = ~binColorsCC(ConsCCLvl), popup = balanceSheet$allData,
                     radius = ~ConsCC*10, group = "Credit Card") %>%
    #biz balances    
    addCircleMarkers(data = balanceSheet, lat = ~Lat, lng = ~Lon, 
                     color = ~binColorsBiz(BizLvl), popup = balanceSheet$allData,
                     radius = ~BizTotal/5, group = "Biz Total") %>%
    
    #Heloc   
    addCircleMarkers(data = balanceSheet, lat = ~Lat, lng = ~Lon, 
                     color = ~binColorsHeloc(HelocLvl), popup = balanceSheet$allData,
                     radius = ~Heloc/10, group = "HELOC") %>%
    ...
    #checking
    addLegend("topleft", pal = binColorsCk, values = balanceSheet$ConsCkLvl,
              title = "Cons Cking",
              opacity = 1) %>%
    ...
    #credit card    
    addLegend("bottomleft", pal = binColorsCC, values = balanceSheet$ConsCCLvl,
              title = "Credit Card",
              opacity = 1) %>%
    
    #new heloc
    addLegend("bottomleft", pal = binColorsHeloc, values = balanceSheet$HelocLvl,
              title = "Heloc",
              opacity = 1) %>%
    
    
    #peer group
    addLegend("bottomright", pal = binColorsInvest, values = balanceSheet$InvestLvl,
              title = "Investments",
              opacity = 1)
