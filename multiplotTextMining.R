...
tokenMatrixFrance <- as.matrix(franceTDM)
tokenMatrixTaiwan <- as.matrix(taiwanTDM)
tokenMatrixUS <- as.matrix(usTDM)
...
#perform multiple plotting of word tokens
#sort it most frequent to least frequent and create a table
tokenV_France <- sort(rowSums(tokenMatrixFrance), decreasing = TRUE)
token.d_France <- data.frame(word = names(tokenV_France), freq=tokenV_France)
table(token.d_France$freq)

tokenV_Taiwan <- sort(rowSums(tokenMatrixTaiwan), decreasing = TRUE)
token.d_Taiwan <- data.frame(word = names(tokenV_Taiwan), freq=tokenV_Taiwan)
table(token.d_Taiwan$freq)

tokenV_US <- sort(rowSums(tokenMatrixUS), decreasing = TRUE)
token.d_US <- data.frame(word = names(tokenV_US), freq=tokenV_US)
table(token.d_US$freq)

#france
png("tokenMultiCloud.png")
par(fig= c(.25,.75,.55,.9))
wordcloud(token.d_France$word, token.d_France$freq,scale=c(4,.5), min.freq = 1, max.words = 500,
          random.order = FALSE,rot.per=0.15, use.r.layout=FALSE,
          colors = brewer.pal(8,"Dark2"))

#taiwan
par(fig= c(0,.45,0,.45), new = TRUE)
wordcloud(token.d_Taiwan$word, token.d_Taiwan$freq,scale=c(4,.5), min.freq = 1, max.words = 500,
          random.order = FALSE,rot.per=0.15, use.r.layout=FALSE,
          colors = brewer.pal(8,"Dark2"))

#us
par(fig= c(.55,1,0,.45), new = TRUE)
wordcloud(token.d_US$word, token.d_US$freq,scale=c(4,.5), min.freq = 1, max.words = 500,
          random.order = FALSE,rot.per=0.15, use.r.layout=FALSE,
          colors = brewer.pal(8,"Dark2"))
