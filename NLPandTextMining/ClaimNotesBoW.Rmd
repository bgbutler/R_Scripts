...
# <strong>Bag of Words Models</strong> {.tabset .tabset-fade .tabset-pills}

## Predicting Large Losses Using Text 'bag of words' models
### Random Forest
### Elastic net (GLMNET)
### Naive Bayes
...
dtm <- DocumentTermMatrix(clm, control = list(weighting = 
                                                    function(x)
                                                    weightTfIdf(x, normalize = FALSE), 
                                                    tokenize = MygramTokenizer))


dtmRed = removeSparseTerms(dtm, 0.998)
...
## Random Forest
```{r rf, echo=T, message=F, warning=F, fig.height=10, fig.width=12}
# Fitting Random Forest Classification to the Training set
classifier = randomForest(x = training_set[,predictorNames],
                          y = training_set[,outcomeName],
                          ntree = 10)

# Predicting the Test set results
y_pred = predict(classifier, newdata = test_set[,predictorNames])

# Making the Confusion Matrix
confusionMatrix(y_pred, test_set[, outcomeName])
...
## Elastic Net
```{r glmnet, echo=T, message=F, warning=F, fig.height=10, fig.width=12}
outcome <- "LL"
...
auc <- roc(ifelse(test_set[,outcome]=="LL",1,0),predictions[[2]])
print(auc$auc)
...
wordRed <- wordGivenLL %>% filter(Prob > .05)
...
wordNotRed <- wordGivenNotLL %>% filter(Prob > .75)
...
## Next Steps
### Improve cleaning and dictionary for common abbreviations
### Determine best architecture to handle data and application
### Develop pipeline and database to hold processed notes for ease of access
### Develop an app/web-app to hold existing code and perform analyses
